...
void control_box_event_update_ui_callback_wrapper(int evnt, uint16_t val)
{
	...
	, ui(new Ui::Dialog_InstrumentConnections)
{
	...
	mod_synth_register_callback_control_box_event_update_ui(&control_box_event_update_ui_callback_wrapper);
...
void Dialog_InstrumentConnections::update()
{
	...
	mod_synth_get_midi_client_connected_to_clients_names_list(client_number, true, &connected_to_midi_clients_names_list);
	...
	if (num_of_connected_jack_input_clients_left > 0)
	{
		...
		for (int j = 0; j < num_of_connected_jack_input_clients_left; j++)
		{
			for (int c = 0; c < 6; c++)
			{
				if (connections_it->c_str() == jack_output_clients_select_pb[c]->text().toStdString())
				{
